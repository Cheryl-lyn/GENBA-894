...
## Predictive model 1: random forest

Step 1: select cols based on previous correlation map
```{r}
rfdata<-amcdata[,c(1,13,14,20,21,22,23,24)]
rfdata[1:3,]

```

Step 2: bar plot of "ISPREMIER"
```{r}
barplot(table(rfdata$ISPREMIER ))
rfdata$ISPREMIER<-as.factor(rfdata$ISPREMIER)
```

Step 3: set training and testing datasets
```{r}
set.seed(123)
samp <- sample(nrow(rfdata), 0.6 * nrow(rfdata))
train <- rfdata[samp, ]
test <- rfdata[-samp, ]
```
...
model <- randomForest(ISPREMIER ~ . , data = train)
```
...
table(pred, test$ISPREMIER)
```

Calculation of Random forest accuracy:
```{r}
(16517+1211) / nrow(test)
```

So, based on the random forest algorithm, the accuracy is 93.96%.

Step 6: features importance
```{r}
gbmImp <- varImpPlot(model)
gbmImp
```
...
```{r}
 library(rpart)
fit <- rpart(ISPREMIER ~ . , data = train,
               method="class")
fit
```
...
```{r}
table(Prediction, test$ISPREMIER)
```
Accuracy

```{r}
(16613+1084)/(16613+1084+1084+87)
```
...
```{r}
mydata <- scale(rfdata[,-1]) # standardize variables
mydata[1:3,]
```
...
library("factoextra")
library("factoextra")
fviz_nbclust(mydata[1:500,], kmeans, method = "gap_stat")

```

```{r}
library("factoextra")
d <- dist(mydata[1:500], method = "euclidean")
